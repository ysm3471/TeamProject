...
closeRoom.forEach(function(item,index){
  ...
  const elSubmenuA = document.querySelector('.main_gnb_M > li:nth-child(3) > a')
  const elDeps = document.querySelector('.main_gnb_M > li:nth-child(3) > ul')

  const showRsv_M = document.querySelector('.showRsv_M')
  const showRsv_MA = document.querySelector('.showRsv_M > a')
  const showRsv_MWrap = document.querySelector('.open_content_wrap')
  const showRsv_ML = document.querySelector('.open_content_left')
  const showRsv_MR = document.querySelector('.open_content_right')
  function updateLayout() {
    if(window.innerWidth < 651){
      activeOFF(elCheckinout,elCal)
      showRsv_M.addEventListener('click',function(){
        elHamBar.classList.remove('on')
        elHamMenu.classList.remove('active')
        elCal.classList.add('active2')
        elCal.style.display = "block"
        showRsv_MWrap.classList.add('active')
        showRsv_ML.classList.add('active2')
        showRsv_MR.classList.add('active2')
      })
    } else {
      elCal.classList.remove('active2')
      elCal.style.display = "none"
      showRsv_MWrap.classList.remove('active')
      showRsv_ML.classList.remove('active2')
      showRsv_MR.classList.remove('active2')
    }
  }
